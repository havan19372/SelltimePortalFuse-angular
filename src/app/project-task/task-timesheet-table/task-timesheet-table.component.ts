...
@Component({
  selector: 'app-task-timesheet-table',
  templateUrl: './task-timesheet-table.component.html',
  styleUrls: ['./task-timesheet-table.component.scss'],
  animations: fuseAnimations
})
export class TaskTimesheetTableComponent implements OnInit,OnDestroy {
  dialogRef: any;
    ...
    this.projectAddDialogRef.afterClosed().subscribe(response => {
      ...
      if (response[0] === 'add') {
        this.ProjectsService.createProject(response[1].getRawValue()).subscribe(
          elem => {
            console.log("@projectCreated",elem);
            debugger;
